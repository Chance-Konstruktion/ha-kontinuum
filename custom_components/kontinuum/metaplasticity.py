...
class MetaPlasticity:
    ...
    def set_params(self, module_name: str, new_values: dict):
        self._init_defaults()
        if module_name not in DEFAULT_PARAMS:
            raise ValueError(f"Unknown module '{module_name}'")
        base = self.data["module_params"].setdefault(module_name, {})
        allowed = set(DEFAULT_PARAMS[module_name].keys())
    ...
    def _collect_metrics(self) -> dict:
        ...
        metrics = {
            ...
            "reticular": {
                ...
                "confidence": ret.get_global_priority() if (ret and hasattr(ret, "get_global_priority")) else 0.5,
            },
            "accumbens": {
                "error": 0.2,
    ...
    def _apply_params_to_modules(self):
        brain = self.hass.data.get("kontinuum", {})
        params = self.data.get("module_params", {})

        hp = brain.get("hippocampus")
        if hp and "hippocampus" in params:
            hp_params = params["hippocampus"]
            if "forgetting_rate" in hp_params:
                hp.DECAY_RATE = max(0.9, min(0.9999, 1.0 - float(hp_params["forgetting_rate"])))

        cb = brain.get("cerebellum")
        if cb and "cerebellum" in params:
            cb_params = params["cerebellum"]
            if "min_confidence" in cb_params:
                cb.MIN_CONFIDENCE = max(0.1, min(0.95, float(cb_params["min_confidence"])))

        bg = brain.get("basal_ganglia")
        if bg and "basal_ganglia" in params:
            bg_params = params["basal_ganglia"]
            if "learning_rate" in bg_params:
                bg.LEARNING_RATE = max(0.01, min(0.3, float(bg_params["learning_rate"])))
            if "q_discount" in bg_params:
                bg.DISCOUNT_FACTOR = max(0.5, min(0.99, float(bg_params["q_discount"])))

        ret = brain.get("reticular")
        if ret and "reticular" in params and "priority_threshold" in params["reticular"]:
            ret.BURST_LIMIT = max(3, min(20, int(12 * (1.0 - float(params["reticular"]["priority_threshold"])))))

        acc = brain.get("accumbens")
        if acc and "accumbens" in params and "reward_scaling" in params["accumbens"]:
            acc.alpha = max(0.05, min(0.5, 0.2 * float(params["accumbens"]["reward_scaling"])))

        loc = brain.get("locus")
        if loc and "locus" in params and "arousal_sensitivity" in params["locus"]:
            sens = max(0.1, min(1.0, float(params["locus"]["arousal_sensitivity"])))
            loc.WINDOW_SECONDS = max(20, min(300, int(120 - sens * 80)))
