...
def _register_services(hass, brain):
    ...
    async def handle_set_meta_params(call):
        ...
        try:
            ...
            _LOGGER.warning("set_meta_params rejected: %s", err)
            return
...
def _update_sensors(hass, brain, last_signal=None, predictions=None):
    ...
    ret = brain["reticular"]
    priority = ret.get_global_priority() if hasattr(ret, "get_global_priority") else 0.5
    if last_signal:
        eid = last_signal.get("entity_id", "")
        if eid:
            priority = ret.get_priority(eid)
    hass.states.async_set("sensor.kontinuum_attention", round(priority, 3), {
